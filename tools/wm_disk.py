...
def build(seed, config, budget, report=None):
    ...
    started = time.time()
    try:
        # There is no address that means "finished" — the World Maker ends by
        # handing control to the game — so this runs on a budget and reports
        # what it got. `run_until` raises when the budget is gone.
        cpu.run_until({0x0000}, max_steps=budget)
    except BaseException:                                   # noqa: BLE001
        pass
    if report:
        report(f"  ran {budget:,} steps in {time.time() - started:.0f}s")
    return cpu, drive
...
def main():
    ...
    cpu, drive = build(seed, args.config, args.budget, report=print)
